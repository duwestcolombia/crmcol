...
/*switch ($rol) {
	...
	$resultado = $db->query("SELECT v.fecha_visita, c.id_cliente,c.nom_cliente, v.nomesporadico_visita,
									u.nom_usuario, z.nom_zona,m.nom_municipio,v.sitzona_visita, c.tipo_cliente,
									v.sitcompetencia_visita, 
									v.rvisita_visita
							FROM visita v, cliente c, usuario u, cliente_municipio cm, municipio m,zona z
							WHERE v.id_usuario = $usuconsulta
							AND v.id_cliente = c.id_cliente
							AND v.id_usuario = u.id_usuario
							AND m.id_municipio = cm.id_municipio
							AND z.id_zona = m.id_zona
							AND v.fecha_visita BETWEEN '$fini' AND '$ffin'
							GROUP BY v.id_visita") or die($db->error);


	$cantDatos = $resultado->num_rows;
	//$cantDatos = $resultado->num_rows();
